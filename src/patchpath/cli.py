...
EVAL_FIXTURES: dict[str, dict[str, Any]] = {
    ...
    "pytest-dev/pytest#14603": {
        "title": "pytest 9.1.0 add handlers to non-propagated loggers",
        "state": "closed",
        "labels": ["status: needs information"],
        "terms": ["handlers", "loggers", "logging"],
    },
}
...
def analyze(
    ...
    terms = filter_search_terms(issue_context["terms"] or plan_search_terms(issue_context))

    matches = run_rg_terms(repo_path, terms, trace)
    apply_codegraph_terms(repo_path, terms, matches, trace, codegraph_runner)
    related_files = rank_files(matches, issue_context)
...
def load_issue_context(
    ...
    labels = [label["name"] for label in (fetched or {}).get("labels", [])] or fixture.get("labels", [])
...
def plan_search_terms(issue_context: dict[str, Any]) -> list[str]:
    ...
    for term in terms:
        ...
        if term not in unique:
            unique.append(term)
    return filter_search_terms(unique)[:12]


def filter_search_terms(terms: list[str]) -> list[str]:
    return [term for term in terms if term.lower() not in GENERIC_SEARCH_TERMS]
...
def rank_files(
    ...
    docs_issue = is_docs_issue(issue_context or {})
    ranked = []
    for item in files.values():
        path = item["path"]
        terms = item["terms"]
        meaningful_terms = {term for term in terms if term.lower() not in GENERIC_SEARCH_TERMS}
        ...
        if is_source_path(path):
            ...
            score += 80 if docs_issue else -24
            if docs_issue and len(meaningful_terms) >= 2:
                score += 20

        if path.endswith((".py", ".js", ".ts", ".d.ts")):
            score += 5
        if not docs_issue and re.search(r"(^|/)(CHANGELOG|CHANGES|docs?)(\.|/|$)", path, re.I):
            score -= 24
...
def is_docs_issue(issue_context: dict[str, Any]) -> bool:
    text = " ".join(
        [
            str(issue_context.get("title", "")),
            " ".join(issue_context.get("labels", [])),
        ]
    ).lower()
    return "doc" in text
...
def is_test_path(path: str) -> bool:
    return path.startswith(("tests/", "test/", "testing/")) or ".test." in path


def is_docs_path(path: str) -> bool:
    return path.startswith(("docs/", "doc/")) or path.lower().startswith("readme")
...
def render_brief(
    ...
    review_only = needs_clarification_or_review(issue_context)
    ...
    lines.extend(
        [
            "",
            "## 澄清/复盘点" if review_only else "## 可能修改点",
        ]
    )
    for item in brief_frame["change_points"]:
        prefix = "复盘/澄清：" if review_only else ""
...
def needs_clarification_or_review(issue_context: dict[str, Any]) -> bool:
    labels = " ".join(issue_context.get("labels", [])).lower()
    return "needs information" in labels or "needs-info" in labels
